...
% ======================= LỌC DÃI QUA TƯƠNG TỰ CẦN THIẾT KẾ ======================
fc1_p = 4;
fc2_p = 11;
OMEGAc1_p = 2*pi*fc1_p;
OMEGAc2_p = 2*pi*fc2_p;
s = OMEGA_c * (sp^2 + OMEGAc2_p*OMEGAc1_p)  /  (sp*(OMEGAc2_p - OMEGAc1_p));
Ha_s = subs(Ha_sTQ,s); % Hàm chuyển của lọc dãi qua cần thiết kế

% =========================== LỌC DÃI QUA SỐ CẦN THIẾT KẾ =========================
% -------------- Hàm truyền đạt H(z)
sz = 2/ts * (1-u)/(1+u);
Hz = subs(Ha_s,sz);
...
f = 0.0001:delta:fs/2-delta+0.0001;
...
xlabel('t'); ylabel('x(t)');
subplot(3,1,3);
plot(t,xL(1:length(t))*167,'m-','linewidth',1.8);
